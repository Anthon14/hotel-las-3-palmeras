...
function limpiarUsuario($texto)
{
    ...
    return substr(trim($texto, '._-'), 0, 30);
}
...
function usuarioExiste($conn, $usuario)
{
    ...
    if (!$consulta) {
        throw new Exception("No se pudo comprobar el usuario.");
    }
...
$nombre = "";
$usuario = "";
$rolNuevo = "Recepcionista";
$rolesPermitidos = ["Recepcionista", "Administrador"];

if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["guardar"])) {
    $csrfRecibido = $_POST["csrf"] ?? "";
    $nombre = trim($_POST["nombre"] ?? "");
    $usuario = limpiarUsuario($_POST["usuario"] ?? "");
    ...
    if (strlen($usuario) < 4 || strlen($usuario) > 30) {
        $errores[] = "El usuario debe tener entre 4 y 30 caracteres.";
    }
    ...
    if (!in_array($rolNuevo, $rolesPermitidos, true)) {
        $errores[] = "Seleccione un rol válido.";
    }

    try {
        if (empty($errores) && usuarioExiste($conn, $usuario)) {
...
if ($consultaTotalUsuarios) {
    $filaTotalUsuarios =
        mysqli_fetch_assoc($consultaTotalUsuarios);

    $totalUsuarios =
        (int) ($filaTotalUsuarios["total"] ?? 0);
}
...
<head>
    ...
    <title>Usuarios - Hotel Las 3 Palmeras</title>
    ...
    <style>
        ...
        .marca-hotel { display: flex; align-items: center; gap: 12px; }
        ...
        .marca-texto small { color: #dbc58f; font-size: 11px; letter-spacing: 1.6px; }
        ...
        .navbar-hotel .nav-link.active { color: white; }
        .usuario-navbar { color: white; font-size: 12px; line-height: 1.2; }
        .usuario-navbar strong { display: block; color: #ead8aa; font-size: 14px; }
        .rol-navbar {
            display: inline-flex;
            margin-top: 4px;
        ...
        .pagina-hero-contenido { max-width: 760px; padding: 70px 0; }
        .pagina-etiqueta { color: #f0d99f; font-size: 12px; font-weight: 900; letter-spacing: 2.5px; }
        .pagina-hero h1 {
            margin: 14px 0 17px;
            font-family: Georgia, serif;
        ...
        .pagina-hero p { max-width: 680px; color: rgba(255,255,255,.82); line-height: 1.7; }

        .contenido-pagina { padding: 75px 0; }
        .seccion-etiqueta { color: #9b7739; font-size: 11px; font-weight: 900; letter-spacing: 2px; }
        .seccion-titulo { color: var(--verde-oscuro); font-family: Georgia, serif; font-weight: 700; }
        .seccion-texto { color: var(--texto-suave); font-size: 14px; line-height: 1.7; }
        ...
        .mensaje-exito { border: 1px solid #b8ddc2; background: #edf8f0; color: #24643a; }
        .mensaje-error { border: 1px solid #edc8c8; background: #fff1f1; color: #9b3131; }
        .mensaje-aviso { border: 1px solid #ead79f; background: #fff8df; color: #765a18; }
        ...
        .formulario-card { margin-bottom: 70px; }
        .formulario-cabecera {
            display: flex;
            align-items: center;
            gap: 14px;
        ...
        .formulario-cabecera h3 { margin: 0; color: var(--verde-oscuro); font-family: Georgia, serif; }
        .formulario-cabecera p { margin: 4px 0 0; color: var(--texto-suave); font-size: 12px; }
        .formulario-cuerpo { padding: 28px; }
        .form-label { font-size: 12px; font-weight: 900; }
        .form-control,
        .form-select {
            min-height: 49px;
        ...
        .form-text { color: var(--texto-suave); font-size: 11px; }
        ...
        .rol-opcion-contenido i {
            font-size: 17px;
        }

        .rol-opcion-contenido strong {
            color: var(--verde-oscuro);
            font-size: 11px;
        }

        .rol-opcion input:checked + .rol-opcion-contenido {
            border-color: var(--verde);
            background: var(--verde-claro);
        ...
        .btn-guardar:hover { background: var(--verde-oscuro); color: white; }
        ...
        .tabla-hotel tbody td { padding: 15px 16px; font-size: 13px; vertical-align: middle; }
        .rol-badge,
        .vinculo-badge,
        .sesion-badge {
            display: inline-flex;
        ...
        .rol-admin { background: #dff2e4; color: #21643b; }
        .rol-recepcion { background: #fff0c7; color: #81600d; }
        .rol-cliente { background: #e2ecff; color: #285eab; }
        .vinculo-badge { background: #f0f1ee; color: #5c655e; }
        .sesion-badge { margin-left: 6px; background: #eee5ff; color: #6947a5; }
        ...
        .btn-editar,
        .btn-eliminar { padding: 8px 11px; border-radius: 4px; font-size: 11px; font-weight: 900; }
        .btn-editar { border: 1px solid #d4b25e; background: #fff5d9; color: #72550d; }
        .btn-eliminar { border: 1px solid #e0a9a9; background: #fff0f0; color: #9d3030; }

        .footer-hotel { background: #13271c; color: white; }
        .footer-principal { padding: 38px 0; }
        ...
        @media (max-width: 767px) {
            .navbar-hotel { min-height: 74px; }
            .navbar-logo { width: 46px; height: 46px; }
            .pagina-hero { min-height: 360px; margin-top: 74px; text-align: center; }
            .contenido-pagina { padding: 55px 0; }
            .formulario-cuerpo { padding: 22px; }
    ...
    <div class="container">
        ...
        <div class="collapse navbar-collapse" id="menuPrincipal">
            <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
                ...
                <li class="nav-item"><a href="../habitaciones/index.php" class="nav-link">Habitaciones</a></li>
                <li class="nav-item"><a href="../reservas/index.php" class="nav-link">Reservas</a></li>
                <li class="nav-item"><a href="../comidas/index.php" class="nav-link">Comidas</a></li>
...
<section class="pagina-hero">
    <div class="container">
        <div class="pagina-hero-contenido">
            <div class="pagina-etiqueta">CONTROL DE ACCESOS</div>
            <h1>Gestión de usuarios</h1>
            <p>
                Administra las cuentas que pueden iniciar sesión. Las cuentas de Cliente
                se crean desde Clientes para que queden relacionadas con un huésped.
            </p>
        </div>
    </div>
</section>
...
<main class="contenido-pagina">
    <div class="container">
        ...
        <?php if (isset($_GET["mensaje"]) && $_GET["mensaje"] === "ultimo_admin") { ?>
            <div class="mensaje mensaje-aviso">
                <i class="bi bi-shield-exclamation"></i>
                No se puede eliminar ni cambiar al último Administrador.
            </div>
        <?php } ?>

        <?php if (isset($_GET["mensaje"]) && $_GET["mensaje"] === "usuario_actual") { ?>
            <div class="mensaje mensaje-aviso">
        ...
        <?php if (isset($_GET["mensaje"]) && $_GET["mensaje"] === "usuario_vinculado") { ?>
            <div class="mensaje mensaje-aviso">
                <i class="bi bi-link-45deg"></i>
                No se puede eliminar la cuenta porque está relacionada con un cliente.
            </div>
        <?php } ?>
        ...
        <section>
            <p class="seccion-etiqueta">NUEVA CUENTA</p>
            <h2 class="seccion-titulo">Registrar trabajador</h2>
            <p class="seccion-texto mb-4">
                Aquí se crean Recepcionistas y Administradores. Para una cuenta de Cliente,
                usa el módulo Clientes.
            </p>

            <div class="formulario-card">
                <div class="formulario-cabecera">
                    <div class="formulario-icono"><i class="bi bi-person-plus"></i></div>
                    <div>
                        <h3>Información de acceso</h3>
                        <p>La contraseña se guardará cifrada.</p>
                    </div>
                </div>
                        ...
                        <div class="row g-4">
                                ...
                                <div class="rol-opciones">
                                    <label class="rol-opcion">
                                        <input
                                            ...
                                            name="rol"
                                            value="Recepcionista"
                                            <?php echo $rolNuevo === "Recepcionista" ? "checked" : ""; ?>
                                            required
                                        >
                                        <span class="rol-opcion-contenido">
                                    ...
                                    <label class="rol-opcion">
                                        <input
                                            ...
                                            name="rol"
                                            value="Administrador"
                                            <?php echo $rolNuevo === "Administrador" ? "checked" : ""; ?>
                                            required
                                        >
                                        <span class="rol-opcion-contenido">
                            ...
                            <div class="col-12">
                                <button type="submit" name="guardar" class="btn-guardar">
                                    <i class="bi bi-person-check"></i>Guardar usuario
                                </button>
                            </div>
                        </div>
                    </form>
        ...
        <section id="usuariosRegistrados">
            <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
                <div>
                    <p class="seccion-etiqueta">CUENTAS REGISTRADAS</p>
                    <h2 class="seccion-titulo">Usuarios del sistema</h2>
                </div>

                <span class="contador">
                    <i class="bi bi-person-gear"></i>
                    <?php echo $totalUsuarios; ?> usuarios
                </span>
            </div>
                        ...
                        <tbody>
                            <?php if ($usuarios && mysqli_num_rows($usuarios) > 0) { ?>
                                <?php while ($row = mysqli_fetch_assoc($usuarios)) { ?>
                                    ...
                                    <tr>
                                        <td><?php echo (int) $row["id_usuario"]; ?></td>

                                        <td>
                                            <strong><?php echo h($row["nombre"]); ?></strong>
                                        ...
                                        <td>
                                            ...
                                            <?php } elseif ($row["rol"] === "Recepcionista") { ?>
                                                <span class="rol-badge rol-recepcion">
                                                    <i class="bi bi-person-workspace"></i>Recepcionista
                                                </span>
                                            <?php } else { ?>
                                                <span class="rol-badge rol-cliente">
                                        ...
                                        <td>
                                            <?php if (!empty($row["id_cliente"])) { ?>
                                                <a
                                                    href="../clientes/editar.php?id=<?php echo (int) $row["id_cliente"]; ?>"
                                                    class="vinculo-badge"
                                                >
                                                    <i class="bi bi-link-45deg"></i>
                                                    Cliente #<?php echo (int) $row["id_cliente"]; ?>
                                                </a>
                                            <?php } else { ?>
                                                <span class="text-muted">Cuenta administrativa</span>
                                            <?php } ?>
                                        </td>
                                        ...
                                        <td>
                                            <div class="acciones">
                                                <a
                                                    href="editar.php?id=<?php echo (int) $row["id_usuario"]; ?>"
                                                    class="btn-editar"
                                                >
                                                ...
                                                <a
                                                    href="eliminar.php?id=<?php echo (int) $row["id_usuario"]; ?>"
                                                    class="btn-eliminar"
                        ...
                        <?php if ($totalPaginas > 1) { ?>
                            ...
                            >
                                ...
                                <?php if ($paginaActual > 1) { ?>
                                    ...
                                    <a
                                        href="?pagina=<?php echo $paginaActual - 1; ?>#usuariosRegistrados"
                                ...
                                <?php if ($paginaInicio > 1) { ?>
                                    ...
                                    <a href="?pagina=1#usuariosRegistrados">1</a>
                                ...
                                <?php for (
                                    ...
                                    <?php } else { ?>
                                        ...
                                        <a href="?pagina=<?php echo $pagina; ?>#usuariosRegistrados">
                                            <?php echo $pagina; ?>
                                        </a>
                                ...
                                <?php if (
                                    ...
                                    <a
                                        href="?pagina=<?php echo $totalPaginas; ?>#usuariosRegistrados"
                                    >
